...
export default function CourseMenu(courseData: CoursePageDTO) {
    // 管理每個 class 的展開/收合狀態
    const [expandedClasses, setExpandedClasses] = useState<Set<number>>(new Set([1])); // 預設第一個展開

    const courseMenuProps: CourseMenuProps = {
        class_titles: courseData.class_titles.map((classItem) => ({
            class_order: classItem.class_order,
            class_name: classItem.class_name,
            chapter_titles: classItem.chapter_titles.map((chapterItem) => ({
                chapter_order: chapterItem.chapter_order,
    ...
    return (
        <Container className="course-menu">
            ...
            {courseMenuProps.class_titles.map((classItem) => (
                            ...
                            <ListGroup variant="flush">
                                {classItem.chapter_titles.map((chapterItem) => (
                                    <ListGroup.Item
                                        ...
                                        onClick={() => {
                                            console.log(`Navigate to Chapter: ${chapterItem.chapter_order}`);
                                        }}
                                    >
